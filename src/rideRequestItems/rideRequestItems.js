...
const RideRequestitems = (props) => {
  ...
  const handleAcceptRider = async () => {
    if (props.avaialableseats > props.riderseats) {
      fetch(`http://localhost:9000/rideRequest/${props.userName}`, {
        method: "PUT",
      ...
      fetch(
        `http://localhost:9000/riderOrders/${props.setDriverOrders.DriverId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            Availableseats: props.avaialableseats - props.riderseats,
          }), // set the new status based on the checkbox value
        }
      )
        .then((response) => {
          if (!response.ok) {
    ...
    <div className="rider-details">
      <div className="grid-items">
        ...
        <div className="button-div">
          <button className="acceptbtn-driver" onClick={handleAcceptRider}>
            ACCEPT
          </button>
          <button className="rejectbtn-driver" onClick={handleRejectRider}>
            REJECT
          </button>
        </div>
      </div>
