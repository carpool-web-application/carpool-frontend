  ...
  const showRideInformation = async () => {
    try {
      const response = await fetch(
        `http://localhost:9000/riderOrders/driver/${props.orders}`
      );
      if (response.ok) {
  ...
  return (
    <div className="driver-details">
      <p className="driver-name">
        {" "}
        Hello {props.driverid}, below are your drive details :{" "}
      </p>
      <p className="driver-name"> {props.origin}</p>
      <p className="driver-name"> {props.destination}</p>
      <p className="driver-name" onClick={displayRiders}>
        {" "}
        {props.orders} is your order number.
      </p>
      <button className="acceptbtn" onClick={handleAcceptRider}>
        Complete the ride
      </button>
      <button className="rejectbtn" onClick={handleRejectRider}>
        Delete the Ride
      </button>

      {<div>((props.status == 'Completed') ? ({rideRequestData}): null)</div>}
    </div>
  );
};
