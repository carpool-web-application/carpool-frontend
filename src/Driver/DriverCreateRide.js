...
const DriverRide = () => {
  ...
  const fetchData = async () => {
    ...
    const existingRecord = await fetch(
      `http://localhost:9000/riderOrders/${parsedData?.userName}`
    );
      ...
      if (!existingRecordData) {
        ...
        if (existingRecordData.length > 0) {
          const recordRequested = existingRecordData.filter(
            (record) => record.DriverPostStatus == "Open"
          );
...

{
  /* 
                <div >
                  ...
                  <Autocomplete>
                    <input name = 'StartingLocation' /* value={destination} /* onChange={onDestinationChange} */
  /* ref={originRef}/>
                  </Autocomplete>
                  */
  /*  <Autocomplete>
                  <input name = 'Destination' /* value={destination} /* onChange={onDestinationChange} */
}
{
  /*ref={destinationRef}/>
                  </Autocomplete> */
